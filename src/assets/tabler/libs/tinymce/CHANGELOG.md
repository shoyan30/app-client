...
- Table classNamees couldn't be removed via setting an empty value in `table_className_list`. Also fixed being forced to pick the first className option. #TINY-6653
...
- SVG icons for back and foreground colors now use `className` instead of `id` to identify SVG elements that should change color. #TINY-9844
...
- Enabling or Disabling checkboxes would not set the correct classNamees and attributes. #TINY-4189
...
- The `tox-custom-editor` className now has a border highlight when it is selected. #TINY-9673
...
- Sticky toolbars did not fade transition when undocking in classNameic iframe mode. #TINY-9408
...
- Renamed the `noneditable_noneditable_className` option to `noneditable_className`. #TINY-8311
- Renamed the `noneditable_editable_className` option to `editable_className`. #TINY-8311
...
- A number of APIs that were not proper classNamees, are no longer constructed using the `new` operator. #TINY-8322
...
- Removed the `imagetools` plugin, which is now classNameified as a Premium plugin. #TINY-8209
- Removed the `imagetools` dialog component. #TINY-8333
- Removed the `toc` plugin, which is now classNameified as a Premium plugin. #TINY-8250
...
- Removed the `shortEnded` and `fixed` properties on `tinymce.html.Node` className. #TINY-8205
- Removed the `mceInsertRawHTML` command. #TINY-8214
- Removed the style field from the `image` plugin dialog advanced tab. #TINY-3422
- Removed the `paste_filter_drop` option as native drag and drop handling is no longer supported. #TINY-8511
- Removed the legacy `mobile` theme. #TINY-7832
- Removed the deprecated `$`, `className`, `DomQuery` and `Sizzle` APIs. #TINY-4520 #TINY-8326
...
- The Table of Contents and Image Tools plugins will be classNameified as Premium plugins in the next major release. #TINY-8087
...
- Added a new `mceTableToggleclassName` command which toggles the provided className on the currently selected table. #TINY-7476
- Added a new `mceTableCellToggleclassName` command which toggles the provided className on the currently selected table cells. #TINY-7476
...
- Updated the `formatter.matchFormat` API to support matching formats with variables in the `classNamees` property. #TINY-7227
- Added HTML5 `audio` and `video` elements to the default alignment formats. #TINY-6633
- Added support for alpha list numbering to the list properties dialog. #TINY-6891

### Changed
- Updated the `image` dialog to display the className list dropdown as full-width if the caption checkbox is not present. #TINY-6400
...
- Added back the ability to use nested items in the `image_className_list`, `link_className_list`, `link_list`, `table_className_list`, `table_cell_className_list`, and `table_row_className_list` settings. #TINY-2236
...
- Fixed an exception thrown when removing inline formats that contained additional styles or classNamees. #TINY-6288
...
- Fixed `fullscreen` plugin now removes all classNamees when the editor is closed. #TINY-4048
...
- Changed inline style element formats (strong, b, em, i, u, strike) to convert to a span on format removal if a `style` or `className` attribute is present. #TINY-4741
...
### Fixed
- Fixed the link dialog such that it will now retain className attributes when updating links. #TINY-2825
...
- Added `tox-dialog__table` instead of `mce-table-striped` className to enhance Help dialog appearance. #TINY-2360
...
### Removed
- Removed the tox-custom-editor className that was added to the wrapping element of codemirror. #TINY-2211
...
- Fixed so that keep_styles set to false no longer clones className/style from the previous paragraph on enter.
...
- Added api to the list plugin for setting custom classNamees/attributes on lists.
...
- Fixed bug where the fullscreen className wouldn't be removed properly when closing dialogs.
...
- Added body_className to template plugin preview. Patch contributed by Milen Petrinski.
...
- Fixed bug where the importcss plugin would import classNamees from the skin content css file.
...
- Fixed bug where resize both option wasn't working due to resent addclassName API change. Patch contributed by Jogai.
...
- Fixed bug where internal classNamees where removed by the removeformat command.
- Fixed bug with creating links table cell contents with a specific selection would throw a exceptions on WebKit/Blink.
- Fixed bug where valid_classNamees option didn't work as expected according to docs. Patch provided by thorn0.
...
### Added
- Added offset/grep to DomQuery className works basically the same as it's jQuery equivalent.
...
- Fixed bug where WebKit/Blink sometimes produced br elements with the Apple-interchange-newline className.
...
- Added new tinymce.util.Color className to handle color parsing and converting.
- Added new colorpicker UI widget element lets you add a hsv color picker to any form/window.
- Added new textpattern plugin that allows you to use markdown like text patterns to format contents.
- Added new resize helper element that shows the current width & height while resizing.
- Added new "once" method to Editor and EventDispatcher enables since callback execution events.
- Added new jQuery like className under tinymce.dom.DomQuery it's exposed on editor instances (editor.$) and globally under (tinymce.$).

### Fixed
- Fixed so the default resize method for images are proportional shift/ctrl can be used to make an unproportional size.
- Fixed bug where the image_dimensions option of the image plugin would cause exceptions when it tried to update the size.
- Fixed bug where table cell dialog className field wasn't properly updated when editing an a table cell with an existing className.
- Fixed bug where Safari on Mac would produce webkit-fake-url for pasted images so these are now removed.
- Fixed bug where the nodeChange event would get fired before the selection was changed when clicking inside the current selection range.
- Fixed bug where valid_classNamees option would cause exception when it removed internal prefixed classNamees like mce-item-.
